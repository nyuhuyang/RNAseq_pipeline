...
sample_table_zhunter<- read.csv("sample_table_zhunter.csv",sep=",",header =T)
...
AnnotatedVcf_zhunter <- read.csv("zhunter_Annotated.eff.vcf",sep="\t",header =T) #Get zhunter_Annotated.eff.vcf
...
t_test <-data.frame()
for(i in 1:nrow(AnnotatedVcf_zhunter_new)){
        t_test[i,1]<-rownames(AnnotatedVcf_zhunter_new)[i]
        mut<-as.double(AnnotatedVcf_zhunter_new[i,12:(11+nrow(zhunter_Sample_ID))])
...
rownames(t_test)<-1:nrow(AnnotatedVcf_zhunter_new)
...
countData <- AnnotatedVcf_zhunter_new[Gene_maker,12:(11+nrow(zhunter_Sample_ID))]
...
ddsHTSeq <- DESeqDataSetFromMatrix(countData=countData,
                                   colData = sample_zhunter.table,
                                   design= ~ Condition)

dds <- DESeqDataSet(ddsHTSeq, design= ~ Condition)
design(dds)
design(dds) <- ~Condition+SPI1
levels(dds$Condition)<-c("Healthy","Cancer")
levels(dds$Condition)
dds <- DESeq(dds)
res_cancer <- results(dds,contrast = c("Condition","Cancer","Healthy"))

par(mfrow=c(3,3))
for (i in 1:8)  plotCounts(dds, order(res_cancer$padj)[i], intgroup="Condition",
                           cex.main = 3,cex=1.5,cex.axis=1.5)
plotCounts(dds, gene="SPI1 47376915", intgroup="Condition",cex.main = 3,cex=1.5,cex.axis=1.5)
...
#detect OS and set enviroment
if (Sys.info()[['sysname']]=="Darwin"){
        FPKM_zhunter_files_path <-paste0("/Users/yah2014/Documents/Programs/R/FPKM/", 
...
dim(FPKM_zhunter)
colnames(FPKM_zhunter) <-zhunter_Sample_name[,1] # or annotation$SimpleLabel
...
write.csv(FPKM_zhunter.gct,"FPKM_zhunter.csv") # open file with excel, delete first column, save as tab delimited text file
...
my_palette3 <- colorRampPalette(c("lightbule2", "orange", "red","green","blue","purple"))(n = 6)



#rename

mut_gene <- AnnotatedVcf_zhunter_novel[,12:(nrow(zhunter_Sample_name)+11)]
for (i in 1:nrow(zhunter_Sample_name)){
        names(mut_gene)[i]<- as.character(zhunter_Sample_name[i,1])#renmae after column 9
}
